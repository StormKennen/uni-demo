...
-->
<template>
  <PageLayout title="Schema Editor" :show-nav="false">
    <view class="sed-page">
      <!-- 顶栏 -->
      ...
      <scroll-view scroll-y class="sed-canvas">
        ...
        <view v-for="(block, idx) in doc" :key="idx" class="sed-row">
          <BlockHost
            :block="block"
            :block-index="idx"
      ...
      <view class="fab-container" v-if="!panelVisible">
        ...
        <view class="fab-actions" :class="{ expanded: fabExpanded }">
          <view v-for="s in allSchemas" :key="s.type" class="fab-action-btn" @click="addBlockAndClose(s.type)">
...
<script setup lang="ts">
  ...
  const editingBlock = computed(() => (editingIndex.value >= 0 ? doc[editingIndex.value] : null))
