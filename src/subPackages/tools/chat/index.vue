...
<script setup lang="ts">
  ...
  const formatMarkdown = (text: string): string => {
    ...
    html = html.replace(/(\|.+\|[\r\n]+\|[-:| ]+\|[\r\n]+(?:\|.+\|[\r\n]*)+)/g, table => {
      ...
      let tableHtml =
        '<div style="overflow-x:auto; -webkit-overflow-scrolling:touch; margin:20rpx 0; border:1rpx solid #e8eaed; border-radius:12rpx;">'
    ...
    html = html.replace(
      /`([^`]+)`/g,
      '<code style="background:#f1f3f4; color:#d93025; padding:2rpx 8rpx; border-radius:6rpx; font-family:monospace; margin:0 4rpx;">$1</code>',
    )
    ...
    html = html.replace(
      /^## (.+)$/gm,
      '<h2 style="margin:25rpx 0 12rpx 0; font-size:34rpx; font-weight:600; color:#202124; border-left:8rpx solid #1a73e8; padding-left:16rpx;">$1</h2>',
    )
    ...
    html = html.replace(
      /^- (.+)$/gm,
      '<div style="margin:10rpx 0; padding-left:20rpx; color:#3c4043; display:flex;"><span style="margin-right:12rpx;">•</span><span>$1</span></div>',
    )
    html = html.replace(
      /^(\d+)\. (.+)$/gm,
      '<div style="margin:10rpx 0; padding-left:20rpx; color:#3c4043; display:flex;"><span style="margin-right:12rpx;">$1.</span><span>$2</span></div>',
    )
    ...
    html = html.replace(
      /^> (.+)$/gm,
      '<div style="border-left:8rpx solid #dfe1e5; padding-left:24rpx; color:#5f6368; margin:15rpx 0; font-style:italic;">$1</div>',
    )
...
<style lang="scss" scoped>
  ...
  .input-area {
    ...
    background: var(--theme-surface);
    box-shadow: 0 -4rpx 20rpx var(--theme-shadow-xs);
    display: flex;
