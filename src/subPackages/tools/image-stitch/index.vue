...
<script setup lang="ts">
  ...
  // 背景颜色选项
  const bgColorOptions = ['#ffffff', '#f5f5f5', '#e0e0e0', '#000000', '#fff3e0', '#e3f2fd', '#f3e5f5', '#e8f5e9']
  ...
  // 颜色选项
  const colorOptions = ['#ffffff', '#000000', '#f5f5f5', '#e0e0e0', '#ff4444', '#44ff44', '#4444ff', '#ffff44']
    ...
    try {
      ...
      while (i < uint8Array.length) {
        if (uint8Array[i] < 0x80) {
          ...
          result += String.fromCharCode(((uint8Array[i] & 0x1f) << 6) | (uint8Array[i + 1] & 0x3f))
          i += 2
        } else {
          result += String.fromCharCode(((uint8Array[i] & 0x0f) << 12) | ((uint8Array[i + 1] & 0x3f) << 6) | (uint8Array[i + 2] & 0x3f))
          i += 3
        }
      }
