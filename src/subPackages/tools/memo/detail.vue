...
<script setup lang="ts">
  ...
  const currentSmartTheme = computed(() => {
    ...
    const isMorandi = ['#E4DCD3', '#D6E4E5', '#F9F5EB', '#E8E4D9', '#D5C4A1', '#C9D6DF', '#E6D5C3'].includes(
  ...
  const renderMarkdown = (content: string, blockStyle?: any, anchorId?: string, isForExport?: boolean): string => {
    ...
    try {
      ...
      html = html
        ...
        .replace(
          /<(p|li|span)(?=\s|>)/gi,
          `<$1 style="color:${color};font-size:${fontSize};font-weight:${fontWeight};font-style:${fontStyle};text-decoration:${textDecoration};line-height:1.6;"`,
        )
        ...
        .replace(
          /<code(?=\s|>)/gi,
          `<code style="color:${color};font-size:${fontSize};padding:2rpx 6rpx;border-radius:4rpx;font-family:monospace;"`,
        )
        .replace(/<pre(?=\s|>)/gi, `<pre style="color:${color};font-size:${fontSize};padding:16rpx;border-radius:8rpx;overflow-x:auto;"`)
        .replace(
          /<blockquote(?=\s|>)/gi,
          `<blockquote style="border-left:4rpx solid #667eea;margin:0;padding:0 16rpx;color:${color};font-size:${fontSize};"`,
        )
  ...
  var isTencentDocUrl = function (url: string): boolean {
    ...
    return (
      trimmedUrl.indexOf('docs.qq.com') !== -1 || trimmedUrl.indexOf('doc.weixin.qq.com') !== -1 || trimmedUrl.indexOf('#小程序://') === 0
    )
  }
...
<style lang="scss" scoped>
  ...
  .route-line {
    ...
    &.line-dashed {
      background: repeating-linear-gradient(to bottom, #ccc 0rpx, #ccc 8rpx, transparent 8rpx, transparent 16rpx);
    }
  ...
  .glass-content {
    ...
    &::before {
      ...
      right: 0;
      height: 2rpx;
      background: linear-gradient(90deg, transparent, rgba(255, 107, 157, 0.6), rgba(116, 185, 255, 0.6), transparent);
    }
