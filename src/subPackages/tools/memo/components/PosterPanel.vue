...
<script setup lang="ts">
  ...
  const drawPoster = () => {
    ...
    ctx.beginPath()
    ctx.arc(CANVAS_W - 50, 50, 110, 0, Math.PI * 2)
    ctx.fill()
    ctx.beginPath()
    ctx.arc(30, CANVAS_H - 60, 80, 0, Math.PI * 2)
    ctx.fill()
    ctx.beginPath()
    ctx.arc(CANVAS_W / 2, CANVAS_H / 2, 180, 0, Math.PI * 2)
    ctx.fill()
