<template>
  <view class="tbr-root" :class="{ 'tbr-selected': selected }" :style="rootStyle" @click="$emit('select', blockIndex)">
    <view class="tbr-header">
      <text class="tbr-tag">📝 文字 #L{{ blockIndex + 1 }}</text>
    </view>

    <view class="tbr-body">
      <view v-for="(item, idx) in block.children" :key="idx" class="tbr-item-row">
        <textarea
          class="tbr-item"
        ...
        <view class="tbr-item-actions">
          <view class="tbr-item-cog" @click.stop="$emit('select-item', blockIndex, idx)">⚙️</view>
          <view class="tbr-item-del" @click.stop="removeItem(idx)">🗑️</view>
        </view>
      </view>
...
<script setup lang="ts">
  ...
  import type { TextBlockData, TextItem } from '../../../schemas'
