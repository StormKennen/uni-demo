<template>
  <view class="abr-root" :class="{ 'abr-selected': selected }" :style="rootStyle" @click="$emit('select', blockIndex)">
    <view class="abr-header">
      <text class="abr-tag">📎 附件 #L{{ blockIndex + 1 }} · {{ block.children.length }} 项</text>
    </view>

    <view v-for="(item, idx) in block.children" :key="idx" class="abr-card">
    ...
    <!-- 资源选择弹窗 -->
    <ResourcePicker v-model:visible="pickerVisible" accept="all" title="选择附件" @confirm="onResourceConfirm" />
  </view>
</template>
