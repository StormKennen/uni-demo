<template>
  <view class="ibr-root" :class="{ 'ibr-selected': selected }" :style="rootStyle" @click="$emit('select', blockIndex)">
    ...
    <view v-if="isGrid" class="ibr-grid" :style="gridStyle">
      <view v-for="(item, idx) in block.children" :key="idx" class="ibr-cell">
    ...
    <!-- carousel 布局 -->
    <swiper v-else-if="isCarousel" class="ibr-swiper" indicator-dots circular :autoplay="false">
    ...
    <!-- 资源选择弹窗 -->
    <ResourcePicker v-model:visible="pickerVisible" accept="image" title="选择图片" @confirm="onResourceConfirm" />
  </view>
</template>
