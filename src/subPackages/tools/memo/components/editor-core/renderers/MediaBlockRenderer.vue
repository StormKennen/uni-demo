<template>
  <view class="mbr-root" :class="{ 'mbr-selected': selected }" :style="rootStyle" @click="$emit('select', blockIndex)">
    <view class="mbr-header">
      <text class="mbr-tag">🎬 多媒体 #L{{ blockIndex + 1 }} · {{ block.children.length }} 项</text>
    </view>

    <view v-for="(item, idx) in block.children" :key="idx" class="mbr-card">
