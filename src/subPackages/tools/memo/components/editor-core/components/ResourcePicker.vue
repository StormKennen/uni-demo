  ...
  <view v-if="visible" class="rp-mask" @click="close">
    <view class="rp-modal" @click.stop>
      ...
      <view class="rp-tabs" v-if="showCloudTab">
        <view class="rp-tab" :class="{ active: activeTab === 'cloud' }" @click="activeTab = 'cloud'">
          <text>☁️ 云端图片</text>
        </view>
        <view class="rp-tab" :class="{ active: activeTab === 'url' }" @click="activeTab = 'url'">
          <text>🔗 外链输入</text>
        </view>
      </view>
      ...
      <view v-if="showCloudTab && activeTab === 'cloud'" class="rp-body">
        ...
        <scroll-view v-else scroll-y class="rp-file-list">
          <view v-for="file in filteredFiles" :key="file.id" class="rp-file-item" @click="selectCloudFile(file)">
            <view class="rp-file-thumb-wrap">
              <image v-if="isImageFile(file.file_name)" :src="file.file_url" class="rp-file-thumb" mode="aspectFill" />
      ...
      <view v-if="activeTab === 'url' || !showCloudTab" class="rp-body rp-url-body">
        <view class="rp-url-field">
          ...
          <input class="rp-url-input" v-model="urlInput" :placeholder="urlPlaceholder" @confirm="confirmUrl" />
        </view>
        <view class="rp-url-field">
          <text class="rp-url-label">名称（可选）</text>
          <input class="rp-url-input" v-model="nameInput" placeholder="输入名称" />
        </view>
...
<script setup lang="ts">
  ...
  const urlPlaceholder = computed(() => {
    switch (props.accept) {
      case 'image':
        return '输入图片链接，如 https://example.com/photo.jpg'
      case 'video':
        return '输入视频链接，如 https://example.com/video.mp4'
      case 'audio':
        return '输入音频链接，如 https://example.com/audio.mp3'
      default:
        return '输入资源链接'
    }
  })
