  ...
  <view class="schema-form">
    <template v-for="field in visibleFields" :key="field.key">
      <view class="schema-field">
        ...
        <!-- radio -->
        <radio-group v-else-if="field.type === 'radio'" class="schema-field-radio-group" @change="onRadioChange(field, $event)">
          <label v-for="opt in field.options || []" :key="String(opt.value)" class="schema-field-radio-item">
            <radio :value="String(opt.value)" :checked="readField(field) === opt.value" color="#667eea" />
        ...
        <view v-else-if="field.type === 'color'" class="schema-field-color-palette">
          <view
...
<script setup lang="ts">
  ...
  import { type SchemaField, getByPath, setByPath } from '../schemas/schema-field'
  ...
  // 可见字段（计算 visible 条件）
  // ============================================================
  const visibleFields = computed(() => props.schema.filter(f => typeof f.visible !== 'function' || f.visible(props.modelValue)))
