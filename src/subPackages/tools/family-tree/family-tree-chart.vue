    ...
    <view class="bottom-panel" :class="{ 'panel-expanded': panelExpanded }">
      ...
      <view class="panel-toggle" :class="{ 'has-selection': selectedNode }" @click="togglePanel">
        <view class="toggle-content">
          <text class="toggle-text" v-if="selectedNode"> <text class="status-indicator">🎯</text> 已选中: {{ selectedNode.name }} </text>
          <text class="toggle-text" v-else> <text class="status-indicator">👆</text> 点击图表节点进行操作 </text>
        </view>
      ...
      <view class="panel-content-wrapper" v-if="panelExpanded">
        ...
        <view class="mode-toggle-float" v-if="selectedNode && isLoggedIn">
          <view class="mode-toggle-btn" :class="{ active: bottomPanelMode === 'detail' }" @click="bottomPanelMode = 'detail'">详情</view>
          <view class="mode-toggle-btn" :class="{ active: bottomPanelMode === 'edit' }" @click="bottomPanelMode = 'edit'">编辑</view>
        </view>
          ...
          <view class="info-section">
            ...
            <view class="info-grid">
              ...
              <view class="info-item">
                ...
                <text class="info-value">{{ selectedNodeGender === 'male' ? '男' : selectedNodeGender === 'female' ? '女' : '未知' }}</text>
              </view>
            </view>
          </view>
...
<script setup>
                  ...
                  children: [
                    {
                      ...
                      value: { gender: 'male', generation: 30 },
                      children: [
                        { id: '6904f112367c7f137d0711e2', name: '梁振辉', value: { gender: 'male', generation: 31 }, children: [] },
                      ],
                    },
                  ],
                },
  ...
  const transformApiDataToTreeData = apiData => {
    ...
    if (apiData && apiData.trees && Array.isArray(apiData.trees) && apiData.trees.length > 0) {
      ...
      // 如果有多棵树，创建虚拟根节点合并
      const transformedTrees = apiData.trees.map(tree => transformNode(tree)).filter(tree => tree !== null)
...
<style lang="scss" scoped>
  ...
  .member-selector {
    ...
    background-color: var(--theme-surface);
    border-radius: 12rpx;
    box-shadow: 0 2rpx 12rpx var(--theme-shadow-sm);
    gap: 20rpx;
  }
  ...
  /* Note: .node-info-toggle already handled above */
  .node-tags {
    display: flex;
    gap: 12rpx;
    margin-top: 8rpx;
