<template>
  <view class="family-tree-list">
    ...
    <view class="search-bar">
      ...
      <input class="search-input" type="text" v-model="searchKeyword" placeholder="搜索成员姓名" @input="onSearch" />
    ...
    <view class="filter-bar">
      <!-- 固定的"全部"标签 -->
      <view class="filter-item fixed-tag" :class="{ active: filterGeneration === null }" @click="filterGeneration = null"> 全部 </view>
    ...
    <view class="generation-dialog-mask" v-if="showGenerationDialog" @click="showGenerationDialog = false">
      <view class="generation-dialog" @click.stop>
        ...
        <view class="dialog-body">
          <input class="dialog-input" type="number" v-model="inputGeneration" placeholder="请输入代数" :focus="showGenerationDialog" />
        </view>
    ...
    <!-- 成员卡片列表 -->
    <scroll-view class="member-list" scroll-y @scrolltolower="onScrollToLower" :lower-threshold="100">
      <!-- 首次加载中 -->
      ...
      <view class="card-grid" v-else>
        <view class="member-card" v-for="member in filteredMembers" :key="member.id" @click="onMemberClick(member)">
          <!-- 卡片头部 -->
...
<style scoped>
  ...
  .member-card {
    ...
    border-radius: 20rpx;
    overflow: hidden;
    box-shadow: 0 4rpx 16rpx var(--theme-shadow-xs);
    transition:
      transform 0.2s ease,
