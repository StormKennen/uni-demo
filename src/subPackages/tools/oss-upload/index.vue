<script setup lang="ts">
  ...
  async function chooseFile() {
    ...
    const filePath = (Array.isArray(res.tempFilePaths) && res.tempFilePaths[0]) || (Array.isArray(res.tempFiles) && res.tempFiles[0]?.path)
    if (filePath) {
      selectedFilePath.value = filePath
  ...
  async function chooseImageAction() {
    ...
    try {
      ...
      const filePath =
        (Array.isArray(res.tempFilePaths) && res.tempFilePaths[0]) || (Array.isArray(res.tempFiles) && res.tempFiles[0]?.path)
