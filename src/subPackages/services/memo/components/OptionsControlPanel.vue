<template>
  <DraftPanelShell
    ...
    <template #tabs>
      <view class="ocp-tabs">
        <view v-for="t in tabs" :key="t.key" class="ocp-tab" :class="{ active: activeTab === t.key }" @click="activeTab = t.key">
          <text>{{ t.label }}</text>
        </view>
      </view>
    </template>
    ...
    <view v-show="activeTab === 'effects'">
      ...
      <view class="settings-section">
        ...
        <view class="setting-item">
          ...
          <switch :checked="draft.settings.showBackToTop" @change="draft.settings.showBackToTop = $event.detail.value" color="#667eea" />
        </view>
        <view class="setting-item">
          <view class="setting-info">
          ...
          <switch :checked="draft.settings.hideNavActions" @change="draft.settings.hideNavActions = $event.detail.value" color="#667eea" />
        </view>
      </view>
    </view>
      ...
      <view class="settings-section">
        ...
        <view class="tags-manager">
          ...
          <view class="tag-add-row">
            <input class="tag-add-input" v-model="tagInput" placeholder="输入标签后回车添加" @confirm="addTag" :maxlength="20" />
