...
<script setup lang="ts">
  ...
  const isItemSelected = (blockIndex: number, itemIndex: number, type: 'text' | 'image') => {
    return selectedItem.value?.blockIndex === blockIndex && selectedItem.value?.itemIndex === itemIndex && selectedItem.value?.type === type
  }
  ...
  const isAnyPanelOpen = computed(() => {
    return configPanelVisible.value || blockStylePanelVisible.value || settingsVisible.value || editingBlockIndex.value !== null
  })
  ...
  const loadCloudFiles = async () => {
    try {
      ...
      if (res.items) {
        // 只显示图片文件
        cloudFiles.value = res.items.filter(file => isImageFile(file.file_name))
      }
      // cloudFiles.value = [
      //   {
      //     id: 'mock-1',
...
<style lang="scss" scoped>
  ...
  .content-block {
    ...
    &.is-locked {
      ...
      outline-offset: -2rpx;
      background: linear-gradient(to right, rgba(230, 162, 60, 0.04), transparent 20%, transparent 80%, rgba(230, 162, 60, 0.04));
    }
    .locked-badge {
      position: absolute;
      top: 12rpx;
  ...
  .node-content {
    ...
    padding: 24rpx;
    margin-bottom: 16rpx;
    border-left: 6rpx solid var(--theme-border);
