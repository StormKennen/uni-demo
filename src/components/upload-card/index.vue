<template>
  <MyCard v-if="type === 'card'">
    ...
    <view class="upload-file-card">
      <view class="file-item" :class="{ border: idx }" v-for="(doc, idx) in signed_document" :key="doc.url" @click="showFileItemPopup(idx)">
        <image class="file-icon" :src="getFileIcon(doc.url)" mode="scaleToFill" />
        <view class="file-name"
          ><slot name="file-name" :scope="{ item: doc, index: idx }">{{ doc.name }}</slot></view
        >
        <uni-icons
          style="transform: rotate(90deg); margin-left: auto"
          type="more-filled"
          size="16"
      ...
      <button
        v-if="!disabled && (!maxCount || signed_document.length < maxCount)"
        class="upload-btn"
        :class="{ 'margin-top': !signed_document.length }"
        @click="upload">
  ...
  <view class="business-card-section upload-file-card-list" v-else-if="type === 'list'">
    ...
    <view
      class="file-item"
      :class="{ border: idx }"
      v-for="(doc, idx) in show_signed_document"
      :key="doc.url"
      ...
      <view class="file-content">
        ...
        <view class="file-upload-at" v-if="doc.upload_at"> 上传于{{ doc.upload_at }} </view>
      </view>
      <uni-icons style="transform: rotate(90deg); margin-left: auto" type="more-filled" size="16" color="rgba(18, 26, 38, 0.5)"></uni-icons>
    </view>
  ...
  <UploadPopup
    ...
    @fail="emit('fail')"
    @beforeSelectFile="emit('beforeSelectFile')"
    :fileTypes="fileTypes"
    private
    v-model="visibleUploadPopup">
  </UploadPopup>
    ...
    <view class="remove-popup-box">
      <view class="remove-popup-content" v-if="visibleConfirmRemovePopup">
        您确定要删除文档<text style="color: #0046b4; word-break: break-all; font-weight: 500">{{
          signed_document[visibleFileItemIndex].name
        }}</text
        >吗？
      </view>
...
<script setup lang="ts">
  ...
  const show_signed_document = computed(() => {
    return (
      signed_document.value?.map(item => {
