  ...
  function getLabel(row: any, prop: any) {
    if (prop && typeof prop === 'string') {
      ...
      const label = propArr.reduce((prev, cur, index) => {
        if (index === propArr.length - 1) prev = isEmpty(prev[cur]) ? '-' : prev[cur]
        else prev = prev[cur] || {}
        return prev
      }, row)
      return label
  ...
  <view
    ...
    <view class="simple-table-header">
      <view
        ...
        :class="[column.fixed ? 'fixed' : '', column.headerAlign || 'center', column.headerClassName ? column.headerClassName : '']"
    ...
    <view v-if="tableData.length" class="simple-table-content">
      <view v-for="(row, index) in tableData" :key="index" class="simple-table-item">
        <view
          v-for="column of columns"
          :key="column.prop"
          class="simple-table-cell"
          :class="[column.fixed ? 'fixed' : '', column.align || 'center', column.cellClassName ? column.cellClassName : '']"
          :style="{ width: `${column.width}rpx` }">
          <slot
            name="cellSlot"
            :column="column"
    ...
    <view class="empty-wrap" v-else> 暂无数据 </view>
  </view>
</template>
