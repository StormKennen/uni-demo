<template>
  <view class="step-bar">
    <view class="step-container" :style="styles?.container">
      <view v-for="(step, i) in data" :key="i" :class="['step', { 'step-last': i === data.length - 1 }]">
        <view class="circle" @click="setStep(i)">
          ...
          <view
            :class="['step-title', { 'current-title': currentStep === i }, { 'completed-title': currentStep > i }]"
            :style="styles?.title">
            {{ step.title }}
          </view>
        </view>
...
<style lang="scss" scoped>
  .step-bar {
    ...
    background: var(--theme-bg); // 背景颜色
  }

  .step-container {
    padding: 56rpx 178rpx 124rpx; // 内边距
    display: flex; // 使用flex布局
    background: var(--theme-surface); // 背景颜色
    ...
    .step {
      ...
      .line {
        height: 0; // 线条高度
        border-bottom: 2px dashed var(--theme-border); // 虚线样式
        flex: 1; // 使线条均匀分布
      ...
      .step-title {
        ...
        color: var(--theme-text-tertiary); // 默认字体颜色
        ...
        &.completed-title {
          color: var(--theme-text); // 已完成步骤标题颜色
        }
      }
    }
