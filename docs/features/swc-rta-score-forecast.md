...
- 首屏并行获取 options/config；当前分数线先返回，趋势图在其后异步加载。当前赛季读取 current/history 展示分界线和日趋势，历史赛季读取 season-history 仅取 `FINAL` 点展示分数线，不展示相对趋势。
- 服务器、赛季或联赛变化时复用 options，仅刷新对应 current/history；目标变化仅刷新对应 history，且预测结果清空。已加载的 config/current/history 在同一页面会话内缓存，刷新时统一失效。
