...
在「魔灵召唤」综合页中，与 RTA 排行榜同级提供 RTA 分数线展示。用户可按后端动态发现的服务器、赛季、联赛、来源平台和目标段位查看最新采集分数线与当前赛季日趋势；历史赛季分数线暂不作为本阶段页面内容，本阶段也不展示或调用预测功能，前端不自行回归或推导分数。
...
- 首屏先获取 options，再用默认服务器/赛季/联赛/来源/目标获取 config、current、history；当前页面只展示最新赛季分界线和日趋势，不请求历史赛季相对曲线。
...
- [ ] current、history 展示数据质量、采集时间、赛季结束和范围核验状态；本阶段不展示 season-history 或 forecast 输入、区间、安全分和置信度。
