...
在「魔灵召唤」综合页中，与 RTA 排行榜同级提供 RTA 分数线展示。用户可按后端动态发现的服务器、赛季、联赛、来源平台和目标段位查看最新采集分数线与当前赛季日趋势；历史赛季使用已落库相对序列的 `FINAL` 点展示结算分数线，不展示趋势图（来源没有可对齐的绝对日期），本阶段不调用预测功能，前端不自行回归或推导分数。
...
- options 是服务器、赛季、联赛、来源平台和目标段位下拉框的唯一事实来源；不得硬编码 `cn`、赛季号或目标 key。赛季或来源切换后带筛选条件重新请求 options。
...
- 首屏先获取 options，再用默认服务器/赛季/联赛/来源/目标获取 config；当前赛季读取 current/history 展示分界线和日趋势，历史赛季读取 season-history 仅取 `FINAL` 点展示分数线，不展示相对趋势。
- 服务器、赛季或联赛变化时重新请求 options，并只保留仍可选的值；目标变化仅刷新 current/history，且预测结果清空。
...
- [x] current、history 与历史赛季 `FINAL` 分数线按数据质量展示；当前赛季展示采集时间/赛季结束，历史赛季明确按相对结算阶段展示；本阶段不展示历史趋势或 forecast 输入、区间、安全分和置信度。
- [ ] `expectedScore=null`、`safeScore=null`、`expectedRankRange=null` 不渲染为 0。
- [ ] 赛季切换重新读取目标枚举，旧请求不能覆盖新筛选。
