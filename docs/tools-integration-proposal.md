...
将「磁力链接补全」「二维码生成/解析」「图片加密」三个工具串联为一条工作流，通过快捷按钮实现一键跳转。
...
| 二维码生成器 | 生成结果下方 | 「加密此图」 | image-cipher |
| 二维码解析 | 解析结果下方 | 「磁力补全」（条件显示） | magnet-link |
| 二维码解析 | 解析结果下方 | 「加密此图」 | image-cipher |
| 图片加密 | 加密结果下方 | 「生成二维码」 | qr-generator |
...
| P1 | 二维码生成 → 图片加密 跳转 | 小（qr-generator 加按钮 + image-cipher 接收参数） |
| P2 | 二维码解析 → 磁力补全 跳转 | 小（qr-parser 加条件按钮 + magnet-link 接收参数） |
| P3 | 图片加密 → 二维码生成 跳转 | 小（image-cipher 加按钮） |
