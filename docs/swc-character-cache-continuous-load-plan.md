...
- 1000+ 魔灵 ⇒ 全量预取会产生 1000+ 张图片下载。当前 `avatar-cache.ts` 的 `MAX_CACHE_ENTRIES=300`（LRU），意味着**持续加载到后面会把前面缓存挤掉**，与「全部预取」目标冲突。
- 小程序限制：`saveFile` 本地文件与 storage 有**总量上限**（约 10MB 量级，随机型不同），无法无脑缓存 1000 张原图。
- 建议策略（择一/组合，待确认）：
  - **a) 提高上限**：将头像缓存上限提升到覆盖默认视图（如「已觉醒」集合规模），并结合默认筛选（觉醒 + 星级倒序）缩小需缓存的集合；
  - **b) 让后端提供缩略图**：头像返回**小尺寸缩略图 URL**（如 `?w=96`），单张体积大幅下降，缓存更多、下载更快（**推荐**，见 §4 后端优化）；
  - **c) 只持久化缩略图、原图走详情页按需**：选人列表用缩略图，点开详情再加载原图。
- H5 端无此容量问题（浏览器 HTTP 缓存），主要针对 mp-weixin。
